...
static int __collate_lookup(wchar_t wc   __LOCALE_PARAM )
{
...
static void __init_collate_state(col_state_t *cs, const Wchar *wcs)
{
...
static void __collate_next_weight(col_state_t *cs, int pass   __LOCALE_PARAM )
{
...
/* 			assert(cs->weightidx != WEIGHT_MASK); */
		...
		BACK_LOOP:
#ifdef WANT_WIDE
			n = 1;
			cs->colitem = r = __collate_lookup(*cs->s   __LOCALE_ARG );
#else  /* WANT_WIDE */
			...
			cs->colitem = r = __collate_lookup(WC   __LOCALE_ARG );
#endif /* WANT_WIDE */
			...
			if (r > CUR_COLLATE->max_col_index) { /* starting char for one or more sequences */
				...
				do {
					...
					do {
						...
						if (!cs->s[n] || (__collate_lookup(cs->s[n]   __LOCALE_ARG ) != *p)) {
							do {} while (*p++);
							break;
						}
						++p;
						...
						if (!cs->s[n] || (__collate_lookup(WC   __LOCALE_ARG ) != *p)) {
							do {} while (*p++);
							break;
						}
						++p;
...
int __XL_NPP(wcscoll) (const Wchar *s0, const Wchar *s1   __LOCALE_PARAM )
{
	...
	do {						/* loop through the weights levels */
		__init_collate_state(ws, s0);
		__init_collate_state(ws+1, s1);
		do {					/* loop through the strings */
			/* for each string, get the next weight */
			__collate_next_weight(ws, pass   __LOCALE_ARG );
			__collate_next_weight(ws+1, pass   __LOCALE_ARG );
			TRACE(("w0=%lu  w1=%lu\n",
...
#warning handle empty string as a special case
	...
	count = pass = 0;
	do {						/* loop through the weights levels */
		__init_collate_state(&cs, ws2);
		do {					/* loop through the string */
			__collate_next_weight(&cs, pass   __LOCALE_ARG );
