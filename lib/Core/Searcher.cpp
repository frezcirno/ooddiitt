...
void DFSSearcher::update(ExecutionState *current,
  ...
  for (auto itr = removedStates.begin(), end = removedStates.end(); itr != end; ++itr) {
    ExecutionState *es = *itr;
    if (es == states.back()) {
      states.pop_back();
    } else {
      auto itr2 = std::find(states.begin(), states.end(), es);
      if (itr2 != states.end()) {
        states.erase(itr2);
      } else {
        assert("invalid state removed");
      }
    }
  }
}
  ...
  if (Instruction *mp = getMergePoint(es)) {
    ...
    if (it==statesAtMerge.end()) {
      ...
      if (mergeWith->merge(es)) {
        ...
        baseSearcher->addState(&es);
        executor.terminateStateOnDispose(es, "merged");
      } else {
  ...
  for (std::map<Instruction*, std::vector<ExecutionState*> >::iterator
    ...
    while (!toMerge.empty()) {
      ...
      for (std::set<ExecutionState*>::iterator it = toErase.begin(),
        ...
        executor.terminateStateOnDispose(**it, "merged");
        toMerge.erase(it2);
      }
